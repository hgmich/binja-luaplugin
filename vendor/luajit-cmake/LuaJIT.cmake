...
# Build the luajit-cmake static library
source_group("include" FILES ${luajit_headers})
source_group("source" FILES ${luajit_sources})
add_library(luajit_lib ${luajit_sources} ${luajit_headers})
set_target_properties(luajit_lib PROPERTIES OUTPUT_NAME luajit)
add_dependencies(luajit_lib
        buildvm_arch_h
        buildvm
        lj_gen_headers
        lj_gen_folddef)
target_include_directories(luajit_lib
        PUBLIC
        $<BUILD_INTERFACE:${LJ_DIR}>
...
if (LIBM_LIBRARIES)
    target_link_libraries(luajit_lib ${LIBM_LIBRARIES})
endif ()

if (LIBDL_LIBRARIES)
    target_link_libraries(luajit_lib ${LIBDL_LIBRARIES})
endif ()

set(LJ_DEFINITIONS ${LJ_DEFINITIONS} -DLUA_MULTILIB="${LUA_MULTILIB}")
target_compile_definitions(luajit_lib PRIVATE ${LJ_DEFINITIONS})
if (IOS)
    set_xcode_property(luajit_lib IPHONEOS_DEPLOYMENT_TARGET "9.0" "all")
endif ()

if ("${LJ_TARGET_ARCH}" STREQUAL "x86")
    if (CMAKE_COMPILER_IS_CLANGXX OR CMAKE_COMPILER_IS_GNUCXX)
        target_compile_options(luajit_lib PRIVATE
...
target_compile_options(luajit_lib PRIVATE ${LJ_COMPILE_OPTIONS})
set_target_properties(luajit_lib PROPERTIES DEBUG_POSTFIX "d")
add_library(luajit::luajit ALIAS luajit_lib)
set_target_properties(luajit_lib PROPERTIES
        FOLDER "luajit"
        INTERFACE_INCLUDE_DIRECTORIES "${LJ_DIR}")

#install(TARGETS luajit_lib
#        EXPORT luajit_lib
